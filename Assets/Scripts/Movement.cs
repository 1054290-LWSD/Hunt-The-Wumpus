...
public class Movement : MonoBehaviour
{
    ...
    //Rigid Body, built in basic physics used for a lot.
    private Rigidbody rb;

    //Max speed at which the player moves
    private float speed = 12f;

    //Gravity rate at which player is pulled down (35f and 35f is pretty good)
    private float gravity = 25f;
    //Strength at which the jump velocity is applied (35f and 35f is pretty good)
    ...
    //Checks in the main update loop if pressed right click, this is used in different loop
    private bool pressedFire = false;
    
    //Checks if is on ground for the dev menu.
    public bool ground = false;

    //The original and changing coyote time variables, coyote time is how long after you fall off a ledge while you can still jump
    private float ogCoyoteTime = 0.1f;
    private float coyoteTime = 0f;

    //The speed at which you dash (50f is pretty good)
    private float dashSpeed = 50f;

    //the values used to determine current camera rotations
